<template>
  <div class="bg-base-200 border border-base-300 p-4 rounded-box gap-3">
    <form @submit.prevent="submitForm">
      <fieldset class="fieldset w-full bg-base-200 border-base-300 p-4 rounded-box gap-3">
        <div v-if="showSuccess" role="alert" class="alert alert-success alert-outline">
          <span>Username has been changed!</span>
        </div>
        <div v-if="error" role="alert" class="alert alert-error alert-outline">
          <span>{{ error }}</span>
        </div>

        <label class="fieldset-label bold">Username</label>
        <input v-model="username" class="input w-full" />
        <span v-if="usernameValidation" class="text-red-500">{{ usernameValidation }}</span>

        <div>
          <button type="submit" class="btn btn-success mt-4 w-max float-right" :disabled="inProgress">Change username</button>
        </div>
      </fieldset>
    </form>
  </div>
</template>

<script lang="ts">
import router from '@/router'
import { mapStores } from 'pinia'
import { HTTP } from '@/utils/http'
import { ERRORS } from '@/utils/errors'
import { useUserStore } from '@/stores/user'
import { useProfileStore } from '@/stores/profile'

export default {
  data() {
    return {
      username: '',
      inProgress: false,
      showSuccess: false,

      error: '',
      usernameValidation: '',
    }
  },
  async mounted() {
    let userStore = useUserStore()
    let user = await userStore.get()

    this.username = user.username.value
  },
  methods: {
    async submitForm() {
      this.inProgress = true

      HTTP.post('/api/account/change-username', {
        username: this.username,
      })
        .then(async (response) => {
          let userStore = useUserStore()
          userStore.username = this.username

          this.error = ''
          this.usernameValidation = ''
          this.showSuccess = true

          await HTTP.post('/api/auth/refresh-token')
        })
        .catch((error) => {
          let response = error.response.data

          this.error = ''
          this.usernameValidation = ''
          this.showSuccess = false

          // Validation errors
          if (response.errors != null) {
            if (response.errors.username != null) {
              this.usernameValidation = ERRORS.get(response.errors.username[0])
            }
          } else if (response.detail != null) {
            this.error = ERRORS.get(response.detail)
          }
        })
        .finally(() => {
          this.inProgress = false
        })
    },
  },
}
</script>
