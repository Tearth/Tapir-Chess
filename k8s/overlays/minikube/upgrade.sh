...
helm upgrade kube-prometheus prometheus-community/kube-prometheus-stack --version 75.6.0 --values ../../base/values/kube-prometheus-values.yaml -n monitoring
helm upgrade loki grafana/loki --version 6.30.1 --values ../../base/values/loki-values.yaml --values ./values/loki-values.yaml -n loki
helm upgrade postgresql cnpg/cloudnative-pg --version 0.24.0 --values ../../base/values/postgresql-values.yaml --values ./values/postgresql-values.yaml -n postgresql
